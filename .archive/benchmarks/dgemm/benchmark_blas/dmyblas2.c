 ...
 *
 * Note:
 *     This is used only when the system lacks an efficient BLAS library.
 */
...
{
    register double vi0, vi1, vi2, vi3, vi4, vi5, vi6, vi7;
	...
	for (k = 0; k < nrow; k++) 
	    Mxvec[k] += vi0 * Mki0[k] + vi1 * Mki1[k]
		      + vi2 * Mki2[k] + vi3 * Mki3[k] 
		      + vi4 * Mki4[k] + vi5 * Mki5[k]
		      + vi6 * Mki6[k] + vi7 * Mki7[k];
